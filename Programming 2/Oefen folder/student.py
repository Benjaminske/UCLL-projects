...
def count(collection, condition):
    return sum(1 for item in collection if condition(item))
...
def count_breed_recursive(dogs, breed):
    if dogs ==[]:
        return 0
    head, *tail = dogs
    return (1 if head.breed == breed else 0) + count_breed_recursive(tail, breed)
...
def count_color_recursive(dogs, color):
    if not dogs:
        return 0
    head, *tail = dogs
    return (1 if head.color == color else 0) + count_color_recursive(tail, color)
...
def find_dog_names_regex(dogs):
    pattern = re.compile(r'^B.*(.)\1.*$')
    return [dog.name for dog in dogs if pattern.match(dog.name)]
...
def count_ancestors(dog, color):
    if dog is None:
        return 0
    father_count = count_ancestors(dog.father, color)
    mother_count = count_ancestors(dog.mother, color)
    own_match = 1 if dog.color == color else 0
    return own_match + father_count + mother_count
...
def print_visual_family_tree(dog, prefix="", is_last=True, label=""):
    ...
    parents = []
    if dog.father:
        parents.append((dog.father, "Father: ", False))
    if dog.mother:
        parents.append((dog.mother, "Mother: ", True))

    for i, (parent, lbl, last) in enumerate(parents):
        is_last_parent = (i == len(parents) - 1)
        print_visual_family_tree(parent, child_prefix, is_last_parent, lbl)

# Voorbeeld ouleh
print_visual_family_tree(pepper)
